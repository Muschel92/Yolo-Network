...
function M.parse(arg)
    ...
    cmd:option('-data_path',        '/data/ethierer/ObjectDetection/YOLO_Network/databases/VOC2007/TrainAndVal/', 'path to dataset')
    ------------- Training options --------------------
    ...
    ---------- Model options ----------------------------------
    cmd:option('-netType',     'resnet-50', 'Options: alexnet | overfeat | alexnetowtbn | vgg | googlenet')
    cmd:option('-netPath',     '/data/ethierer/ObjectDetection/YOLO_Network/model/', 'Path to the Yolo model')
    cmd:option('-retrain',     'none', 'provide path to model to retrain with')
    cmd:option('-new_model',     true, 'if a new model should be created')
    ----------- Saving Options ----------------------------------
